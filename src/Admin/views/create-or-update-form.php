...
<div class="wrap">
    ...
    <h1 id="add-new-user"><?php echo esc_html($view->pageTitle()) ?></h1>
        ...
        <table class="form-table" role="presentation">
            <tbody>
                <tr class="form-field form-required">
                    ...
                    <td><input name="name" type="text" id="name" value="<?php echo esc_html($view->formName()) ?>" aria-required="true" size="30" required></td>
                </tr>
                        ...
                        <select name="email-list" id="email-list">
                            <?php foreach ($view->emailLists() as $emailList) { ?>
                                <option
                                    ...
                                    value="<?php echo esc_html($emailList->uuid) ?>"
                                ><?php echo esc_html($emailList->name) ?></option>
                            <?php } ?>
                        </select>
        ...
        <table class="form-table">
            <tbody>
                <tr class="form-field form-required">
                    ...
                    <td><input name="message-subscribed" type="text" id="message-subscribed" value="<?php echo esc_html($view->messages()->subscribed) ?>" required></td>
                </tr>

                <tr class="form-field form-required">
                    <th scope="row"><label for="message-pending">Message when pending subscription</label></th>
                    <td><input name="message-pending" type="text" id="message-pending" value="<?php echo esc_html($view->messages()->pending) ?>" required></td>
                </tr>

                <tr class="form-field form-required">
                    <th scope="row"><label for="message-already-subscribed">Message when already subscribed</label></th>
                    <td><input name="message-already-subscribed" type="text" id="message-already-subscribed" value="<?php echo esc_html($view->messages()->alreadySubscribed) ?>" required></td>
                </tr>
            </tbody>
        </table>
